...
const parse=doc=>{document=doc;offset=0;output=[];stack=[];tokenizer.lastIndex=0;do{}while(proceed());return output;};function proceed(){const stackDepth=stack.length;const next=nextToken();if(next===null){if(0===stackDepth){addFreeform();return false;}
if(1===stackDepth){addBlockFromStack();return false;}
while(0<stack.length){addBlockFromStack();}
return false;}
const[tokenType,blockName,attrs,startOffset,tokenLength]=next;const leadingHtmlStart=startOffset>offset?offset:null;switch(tokenType){case 'void-block':if(0===stackDepth){if(null!==leadingHtmlStart){output.push(Freeform(document.substr(leadingHtmlStart,startOffset-leadingHtmlStart)));}
...
function nextToken(){const matches=tokenizer.exec(document);if(null===matches){return null;}
